...
# files = [f for f in listdir("results/astarGrid")]
...
#     dfpap = dfpaps[j]
#     df = pd.read_csv("results/astarGrid/" + files[i], header=5)
#     df2 = pd.read_csv("results/astarGrid/" + files[i + 143], header=5)
#     df3 = pd.read_csv("results/astarGrid/" + files[i + 143 * 2], header=5)
#     print(str(dfpap) + "," 
#           + '%.2f' % dfpmp + "," + 
#           '%.2f' % ((df["win/fail"].sum() + df2["win/fail"].sum() + df3["win/fail"].sum()) / 429) + "," +
#           '%.2f' % ((df["% travelled"].sum() + df2["% travelled"].sum() + df3["% travelled"].sum()) / 429) + "," +
...
df = pd.read_csv("resultsConc/astarGrid.csv")
